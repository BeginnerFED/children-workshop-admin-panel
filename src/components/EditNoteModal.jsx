...
export default function EditNoteModal({ isOpen, onClose, onSuccess, note }) {
  ...
  // Not verisi geldiğinde form verilerini güncelle
  useEffect(() => {
    if (note) {
      setFormData({
        title: note.title || '',
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      await onSuccess({ ...formData, id: note.id })
      onClose()
    } catch (error) {
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm sm:text-base text-[#6e6e73] dark:text-[#86868b]">
                    {language === 'tr' ? 'Notu düzenle ve kaydet' : 'Edit and save the note'}
                  </p>
                </div>
              </div>
            </div>
